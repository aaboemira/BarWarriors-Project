  ...
  <table>       
    <thead>
      <tr id="table">
        ...
        <th class="th-sm" id="plans">Productimg</th>
      <button onclick="add('<?= $name?>'),add('<?= $name?>'),add('<?= $name?>'),add('<?= $name?>')" >clickme</button>
      <script type="text/javascript">         
      function add($item){
      var item=document.createElement('td');
      item.className='th-sm';
      item.innerHTML=$item;
   ...
   <!--  <tbody>
     <tr>
        ...
        <td id=""><?=$quantity?></td>
        <td id=""><?= $total?></td>
        <td id=""><?= $stored_quantity?></td>
        <td id=""><img src="img/prod<?=$img?>" style="width: 50px;"></td>
        <td class="th-sm" id="plans"><button type="submit" name="submit" value="ordered">order</button></td>
      </tr></tbody>!-->
  </table>
</div>
</div>
